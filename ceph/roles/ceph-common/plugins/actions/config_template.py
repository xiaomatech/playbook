...
try:
    ...
    def _convert_2_string(item):
        ...
        if isinstance(item, dict):
            ...
            return dict((_convert_2_string(key), _convert_2_string(value))
    ...
    class ActionModule(ActionBase):
        ...
        def return_config_overrides_ini(config_overrides, resultant):
            ...
            for section, items in config_overrides.items():
                ...
                if not isinstance(items, dict):
                    config.set('DEFAULT', section.encode('utf-8'),
                    ...
                    for key, value in items.items():
                        ...
                        try:
                            config.set(
                                section.encode('utf-8'), key.encode('utf-8'),
                                value)
        ...
        def return_config_overrides_json(self, config_overrides, resultant):
            ...
            merged_resultant = self._merge_dict(base_items=original_resultant,
        ...
        def return_config_overrides_yaml(self, config_overrides, resultant):
            ...
            merged_resultant = self._merge_dict(base_items=original_resultant,
                                                new_items=config_overrides)
            return yaml.safe_dump(merged_resultant,
        ...
        def _load_options_and_status(self, task_vars):
            ...
            if faf:
                ...
                source = self._loader.path_dwim_relative(
                    file_path, 'templates', user_source)
                searchpath.insert(1, os.path.dirname(source))

            _dest = self._task.args.get('dest')
            if not _dest:
                return False, dict(failed=True, msg="No [ dest ] was provided")
            else:
                # Expand any user home dir specification
                user_dest = self._remote_expand_user(_dest)
                if user_dest.endswith(os.sep):
                    user_dest = os.path.join(user_dest,
                                             os.path.basename(source))

            return True, dict(source=source,
                              dest=user_dest,
                              config_overrides=self._task.args.get(
        ...
        def run(self, tmp=None, task_vars=None):
            ...
            try:
                template_uid = temp_vars['template_uid'] = pwd.getpwuid(
                    os.stat(source).st_uid).pw_name
            except Exception:
                template_uid = temp_vars['template_uid'] = os.stat(
                    source).st_uid

            managed_default = C.DEFAULT_MANAGED_STR
            managed_str = managed_default.format(host=template_host,
                                                 uid=template_uid,
            ...
            resultant = self._templar.template(resultant,
                                               preserve_trailing_newlines=True,
                                               escape_backslashes=False,
                                               convert_data=False)
            ...
            # Run the copy module
            return self._execute_module(module_name='copy',
        ...
        def run(self,
            ...
            if '_original_file' in inject:
                source_file = utils.path_dwim_relative(
                    inject['_original_file'], 'templates', source_template,
                    self.runner.basedir)
            else:
                source_file = utils.path_dwim(self.runner.basedir,
                                              source_template)
