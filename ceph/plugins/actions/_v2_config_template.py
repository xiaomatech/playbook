...
class ActionModule(ActionBase):
    ...
    def _load_options_and_status(self, task_vars):
        ...
        if faf:
            ...
            if not user_source:
                return False, dict(failed=True,
                                   msg="No user provided [ src ] was provided")
            source = self._loader.path_dwim_relative(file_path, 'templates',
                                                     user_source)
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            template_uid = temp_vars['template_uid'] = pwd.getpwuid(os.stat(
                source).st_uid).pw_name
        except Exception:
        ...
        if _vars['config_overrides']:
            ...
            resultant = type_merger(config_overrides=_vars['config_overrides'],
                                    resultant=resultant)
        ...
        new_module_args.update(dict(src=transferred_data,
                                    dest=_vars['dest'],
